...
function updateStateInternal(context) {
    ...
    // Dynamic allocation
    const staticColumns = cache('staticColumns', getStaticColumns, [sortedColumns]);
    const staticRows = cache('staticRows', getStaticRows, [sortedRows]);

    // Shaping
    const shapedColumns = staticColumns;
    const shapedRows = staticRows;
    // Measuring
