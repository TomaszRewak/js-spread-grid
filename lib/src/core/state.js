...
function updateStateInternal(context) {
    ...
    function cache(key, func, dependencies) {
        ...
        if (!previousDependencies || dependencies.some((dependency, index) => dependency !== previousDependencies[index])) {
            memory[key] = /** @type {Types.Memory[K]} */ ({ value: func(...dependencies), dependencies });
    ...
    const mousePosition = cache_value_deep('mousePosition', context.mousePosition);
    ...
    const clientSize = cache_value_deep('clientSize', getClientSize(element));
    const scrollOffset = cache_value_deep('scrollOffset', getScrollOffset(element));
    const internalMousePosition = cache('internalMousePosition', getInternalPosition, [mousePosition, clientSize, scrollOffset, fixedSize, totalSize]);
    const scrollRect = cache_value_deep('scrollRect', getScrollRect(previousState?.scrollRect, totalSize, fixedSize, element));
    const activeHorizontalScroll = cache('activeHorizontalScroll', getActiveHorizontalScroll, [options.horizontalScrollTarget, measuredColumns, pinning, internalMousePosition, fixedSize, totalSize, clientSize, scrollOffset, borderWidth]);
    const activeVerticalScroll = cache('activeVerticalScroll', getActiveVerticalScroll, [options.verticalScrollTarget, measuredRows, pinning, internalMousePosition, fixedSize, totalSize, clientSize, scrollOffset, borderWidth]);
    ...
    const hoveredCell = cache_value_deep('hoveredCell', getHoveredCell(internalMousePosition, rows, columns, fixedSize, totalSize));
