...
import { getFrozenColumns, getFrozenRows } from "../state-utils/getFrozen.js";
...
export default function updateState(context) {
    ...
    const freezeColumnsOnHover = options.freezeOnHover && !stateChanges.includes('sortBy') && !stateChanges.includes('filters') && !stateChanges.includes('columnWidths') && !stateChanges.includes('columnsOrder');
    const freezeRowsOnHover = options.freezeOnHover && !stateChanges.includes('sortBy') && !stateChanges.includes('filters') && !stateChanges.includes('rowHeights') && !stateChanges.includes('rowsOrder');
    const shapedColumns = cache('shapedColumns', getFrozenColumns, [sortedColumns, freezeColumnsOnHover, previousState?.hoveredCell, previousState?.shapedColumns]);
    const shapedRows = cache('shapedRows', getFrozenRows, [sortedRows, freezeRowsOnHover, previousState?.hoveredCell, previousState?.shapedRows]);
