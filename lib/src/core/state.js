...
function updateStateInternal(context) {
    ...
    const unfilteredColumns = cache('unfilteredColumns', getResolvedColumns, [unfoldedColumns, options.pinnedLeft, options.pinnedRight, options.columnWidths, borderWidth, devicePixelRatio]);
    const unfilteredRows = cache('unfilteredRows', getResolvedRows, [unfoldedRows, options.pinnedTop, options.pinnedBottom, options.rowHeights, borderWidth, devicePixelRatio]);
    ...
    // Scrolling
    const fixedSize = cache('fixedSize', getFixedSize, [measuredColumns, measuredRows, borderWidth]);
    ...
    // Dynamic allocation
    const staticColumns = cache('staticColumns', getStaticColumns, [measuredColumns]);
    const staticRows = cache('staticRows', getStaticRows, [measuredRows]);

    // Placement
    const columns = cache('columns', getPlacedColumns, [staticColumns, borderWidth]);
    const rows = cache('rows', getPlacedRows, [staticRows, borderWidth]);
