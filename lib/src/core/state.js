...
function updateStateInternal(context) {
    ...
    function cache_callback(key, value, callback) {
        const previousValue = memory[key] && memory[key].value;
    ...
    cache_callback('activeColumnsCallback', activeColumns, options.onActiveColumnsChange);
    cache_callback('activeRowsCallback', activeRows, options.onActiveRowsChange);
    cache_callback('hoveredCellCallback', hoveredCell, options.onHoveredCellChange);
