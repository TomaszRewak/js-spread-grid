...
function updateStateInternal(context) {
    ...
    const measureFormatResolver = cache('measureFormatResolver', getFormatResolver, [measureFormattingRules, data, filteredRows, filteredColumns, edition]);
    const columnWidthCache = cache('columnWidthCache', () => new Map(), []);
    const rowHeightCache = cache('rowHeightCache', () => new Map(), []);
    const measuredColumns = cache('measuredColumns', getMeasuredColumns, [filteredColumns, filteredRows, textResolver, measureFormatResolver, columnWidthCache, unfilteredColumnKeys]);
    const measuredRows = cache('measuredRows', getMeasuredRows, [filteredColumns, filteredRows, textResolver, measureFormatResolver, rowHeightCache, unfilteredRowKeys]);
