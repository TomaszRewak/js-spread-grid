...
import { getActiveHorizontalScroll, getActiveVerticalScroll } from "../state-utils/getActiveScroll.js";
import stringifyId from "../core-utils/stringifyId.js";

/** @type {Set<keyof Types.Memory>} */
const visuallyImportantFields = new Set([
    'sections',
    'devicePixelRatio',
    'scrollRect',
...
/**
 * @param {Types.ActiveScroll} a
 * @param {Types.ActiveScroll} b
 * @returns {boolean}
 */
function compareActiveScroll(a, b) {
    if (!a && !b)
        return true;
    if (!a || !b)
        return false;
    return a.offset === b.offset && a.snap === b.snap && a.speed === b.speed;
}
...
function updateStateInternal(context) {
    ...
    function cache(key, func, dependencies, comparator = (a, b) => a === b) {
        ...
        if (!previousDependencies || dependencies.some((dependency, index) => dependency !== previousDependencies[index])) {
            ...
            if (!comparator(previousValue, newValue)) {
                memory[key] = /** @type {Types.Memory[K]} */ ({ value: newValue, dependencies });
                stateChanges.push(key);
                //console.log('State change:', key);
                if (visuallyImportantFields.has(key)) {
                    //console.log('Visual change due to', key);
                    context.hasVisualChanges = true;
                }
            } else {
                memory[key] = /** @type {Types.Memory[K]} */ ({ value: previousValue, dependencies });
            }
        }
    ...
    const unfilteredColumns = cache('unfilteredColumns', getResolvedColumns, [unfoldedColumns, options.columnWidths, borderWidth, devicePixelRatio]);
    const unfilteredRows = cache('unfilteredRows', getResolvedRows, [unfoldedRows, options.rowHeights, borderWidth, devicePixelRatio]);
    ...
    const scrollRect = cache('scrollRect', getScrollRect, [previousState?.scrollRect, scrollOffset, totalSize, fixedSize, boundingClientSize], compareRects);
    const activeHorizontalScroll = cache('activeHorizontalScroll', getActiveHorizontalScroll, [options.horizontalScrollTarget, measuredColumns, pinning, internalMousePosition, fixedSize, totalSize, clientSize, scrollOffset, borderWidth], compareActiveScroll);
    const activeVerticalScroll = cache('activeVerticalScroll', getActiveVerticalScroll, [options.verticalScrollTarget, measuredRows, pinning, internalMousePosition, fixedSize, totalSize, clientSize, scrollOffset, borderWidth], compareActiveScroll);
