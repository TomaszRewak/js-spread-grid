...
function updateStateInternal(context) {
    ...
    const unfilteredColumns = cache('unfilteredColumns', getResolvedColumns, [invokedColumns, options.pinnedLeft, options.pinnedRight]);
    const unfilteredRows = cache('unfilteredRows', getResolvedRows, [invokedRows, options.pinnedTop, options.pinnedBottom]);
