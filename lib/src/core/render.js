...
function scroll(context) {
    const state = context.state;
    const element = context.element;
    const activeHorizontalScroll = state.activeHorizontalScroll;;
    const activeVerticalScroll = state.activeVerticalScroll;

    if (activeVerticalScroll) {
        element.scrollTo({
            top: activeVerticalScroll.offset,
            behavior: activeVerticalScroll.snap ? 'smooth' : 'auto',
        });
    }
}
