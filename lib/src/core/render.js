...
function scroll(context) {
    ...
    // Animated scrolling at speed pixels/second
    ...
    const animate = (now) => {
        ...
        //console.log({ position, step, diff, speed, dt });

        element.scrollTop = position;

        //console.log({ elementScroll: element.scrollTop });

        const atTarget = Math.abs(diff) < 1;
        const maxScroll = element.scrollHeight - element.clientHeight;
        const atLimit = (position <= 0 && diff < 0) || (position >= maxScroll && diff > 0);
