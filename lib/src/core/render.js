...
function renderSection(context, vertical, horizontal) {
    ...
    for (let columnIndex = minVisibleColumnIndex; columnIndex <= maxVisibleColumnIndex; columnIndex++) {
        for (let rowIndex = minVisibleRowIndex; rowIndex <= maxVisibleRowIndex; rowIndex++) {
            ...
            const foreground = style.foreground || 'black';
            ...
            if (text && foreground !== 'transparent') {
...
function scroll(context) {
    ...
    let lastScrollTo = /** @type {number|null} */ (null);
    ...
    const animate = (now) => {
        ...
        if (target.snapVersion !== context.scrollSnapVersion) {
            context.scrollSnapVersion = target.snapVersion;
            lastScrollTo = null;
            if (element.scrollTop !== target.offset) {
                //console.log('scroll to (snap)', target.offset, ' version:', target.snapVersion);
                element.scrollTo({ top: target.offset, behavior: 'auto' });
            }
        } else if (!speed) {
            if (lastScrollTo !== target.offset || element.scrollTop === previousScrollTop && element.scrollTop !== target.offset) {
                //console.log('scroll to', target.offset);
                element.scrollTo({ top: target.offset, behavior: 'smooth' });
            }
            lastScrollTo = target.offset;
            previousScrollTop = element.scrollTop;
        } else {
            const position = element.scrollTop;
            const diff = target.offset - position;

            if (Math.abs(diff) >= 1) {
                const step = diff > 0
                    ? Math.min(diff, speed * dt + context.fractionalScrollProgress)
                    : Math.max(diff, -speed * dt + context.fractionalScrollProgress);
                const stepRounded = step > 0 ? Math.floor(step) : Math.ceil(step);
                context.fractionalScrollProgress = step - stepRounded;
                lastScrollTo = null;

                if (stepRounded) {
                    element.scrollBy({ top: stepRounded, behavior: 'auto' });
                }
            }
        }
...
function renderInternal(context) {
    ...
    // TODO: Move those to separate functions outside of render so that we can skip rendering grid when only those change
    renderInput(context);
