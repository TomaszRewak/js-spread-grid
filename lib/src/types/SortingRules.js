...
export default class SortingRules {
    ...
    constructor(rules) {
        ...
        for (const rule of rules) {
            const userComparatorAsc = rule.comparatorAsc || rule.comparator;
            const userComparatorDesc = rule.comparatorDesc || (rule.comparator ? (lhs, rhs) => -toComparisonResult(rule.comparator(lhs, rhs)) : null);

            /** @type {Types.SortingComparator} */
            const comparatorAsc = userComparatorAsc
                ? (lhs, rhs) => userComparatorAsc(lhs, rhs)
                : (lhs, rhs) => defaultComparatorAsc(lhs, rhs);
            /** @type {Types.SortingComparator} */
            const comparatorDesc = userComparatorDesc
                ? (lhs, rhs) => userComparatorDesc(lhs, rhs)
