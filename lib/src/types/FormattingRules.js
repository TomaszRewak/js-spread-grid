...
/** @type {("borderTop" | "borderRight" | "borderBottom" | "borderLeft")[]} */
...
/**
 * @param {Types.FunctionContext} context
 */
...
/**
 * @param {Types.FormattingRule} rule
 * @param {Types.FunctionContext} context
 * @returns {Types.Edit}
 */
...
export default class FormattingRules {
    /**
     * @param {Types.Rule[]} rules
     */
    constructor(rules) {
        /** @type {RulesLookup<Types.FormattingRule>} */
        this.rulesLookup = new RulesLookup();

        for (const [index, rule] of rules.entries()) {
            /** @type {Types.FormattingRule} */
            const entry = { index };
            ...
            if ('sortOrder' in rule)
                entry.sortOrder = getResolvedFunction(rule.sortOrder);
    ...
    resolve(data, rows, columns, row, column, edition) {
        ...
        /** @type {Types.FunctionContext} */
        let context = { data, rows, columns, row, column };
        let style = {};
        /** @type {Types.InternalDrawFunction} */
        let draw = undefined;
        ...
        for (const rule of rules) {
            ...
            if ('sortOrder' in rule)
                context = { ...context, sortOrder: rule.sortOrder(context) };
            if ('draw' in rule) {
        ...
        if ('sortOrder' in context)
            result.sortOrder = context.sortOrder;
        if (draw !== undefined)
            result.draw = draw;
