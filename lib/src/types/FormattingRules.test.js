...
describe('FormattingRules', () => {
  describe('resolve', () => {
    it('should return an empty object when no styles are matched', () => {
      ...
      const style = resolver.resolve(null, [], [], { key: stringifyId('row3') }, { key: stringifyId('col3') }, new Edition([]), new Filtering([])).style;
    ...
    it('should apply indexing to border styles', () => {
      ...
      const resolver = new FormattingRules(styles);
      const style = resolver.resolve(null, [], [], { key: stringifyId('row1') }, { key: stringifyId('col1') }, new Edition([]), new Filtering([])).style;
      expect(style).toEqual({
    ...
    it('should use the value for evaluating conditions', () => {
      ...
      const resolver = new FormattingRules(styles);
      const style = resolver.resolve(null, [], [], { key: stringifyId('row1'), data: true }, { key: stringifyId('col1') }, new Edition([]), new Filtering([])).style;
      expect(style).toEqual({ color: 'red' });
    });
    ...
    it('should use ANY rules for matching undefined columns and rows', () => {
      ...
      const style = resolver.resolve(null, [], [], { key: stringifyId('row3'), type: 'DATA' }, { key: stringifyId('col3'), type: 'DATA' }, new Edition([]), new Filtering([])).style;
      expect(style).toEqual({ color: 'green' });
    });

    it('should use latest matching rule', () => {
      const resolver = new FormattingRules(commonStyles);
      const style = resolver.resolve(null, [], [], { key: stringifyId('row1'), index: 1, type: 'DATA' }, { key: stringifyId('col1'), index: 1, type: 'DATA' }, new Edition([]), new Filtering([])).style;
      expect(style).toEqual({ color: 'purple' });
    });

    it('combine styles from multiple rules', () => {
      const resolver = new FormattingRules(commonStyles);
      const style = resolver.resolve(null, [], [], { key: stringifyId('row2'), index: 2, type: 'DATA' }, { key: stringifyId('col2'), index: 2, type: 'DATA' }, new Edition([]), new Filtering([])).style;
      expect(style).toEqual({ color: 'brown', background: 'black' });
    });

    it('should use index rules for matching columns and rows by index', () => {
      const resolver = new FormattingRules(commonStyles);
      const style = resolver.resolve(null, [], [], { key: stringifyId('row2'), index: 2, type: 'DATA' }, { key: stringifyId('col0'), index: 0, type: 'DATA' }, new Edition([]), new Filtering([])).style;
