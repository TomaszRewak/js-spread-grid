import { contains, clip, expand, area, remove } from './rect.js';
...
describe('remove', () => {
    it('should remove the margin from the rectangle', () => {
        const rect = { top: 10, left: 20, width: 30, height: 40 };
        const margin = { top: 5, right: 10, bottom: 5, left: 10 };
        const expected = { top: 10, left: 20, width: 10, height: 30 };
        expect(remove(rect, margin)).toEqual(expected);
    });

    it('should handle negative values when removeing the margin', () => {
        const rect = { top: 10, left: 20, width: 30, height: 40 };
