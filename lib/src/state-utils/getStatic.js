...
/**
 * @param {Types.ResolvedDimension[]} elements
 * @returns {boolean}
 */
...
/**
 * @param {(Types.ResolvedStaticDimension | Types.ResolvedDynamicDimension)[]} elements
 * @returns {number}
 */
...
export function getStaticColumns(data, columns, pinning, scrollRect, fixedSize, borderWidth) {
    ...
    const staticColumns = /** @type {Types.MeasuredStaticColumn[]} */ ([]);
    const addColumn = (/** @type {number} */ snap, /** @type {Types.MeasuredStaticColumn} */ column) => {
        staticColumns.push(column);
        currentLeft += column.width + borderWidth;
        globalColumnIndex += snap;
    }
    const addStaticColumn = (/** @type {Types.MeasuredStaticColumn} */ column) => {
        addColumn(1, column);
    }
    const addDataColumns = (/** @type {Types.MeasuredDynamicColumn} */ column, /** @type {number} */ start, /** @type {number} */ end) => {
        for (let index = start; index <= end; index++) {
            const selector = column.selector({ data, index });
            const id = column.id({ data, selector });
            addColumn(1, {
                ...column,
                type: /** @type {Types.StaticDimensionType} */ ('DATA'),
                id,
                key: stringifyId(id),
                selector,
    ...
    const addSeparator = (/** @type {Types.MeasuredDynamicColumn} */ column, /** @type {number} */ span) => {
        if (span > 0) {
            const id = `__separator_${separatorCount++}`;
            addColumn(span, {
                ...column,
                type: /** @type {Types.StaticDimensionType} */ ('SEPARATOR'),
                id,
                key: stringifyId(id),
                header: '',
...
export function getStaticRows(data, rows, pinning, scrollRect, fixedSize, borderWidth) {
    if (!hasDynamicBlocks(rows))
        return /** @type {Types.MeasuredStaticRow[]} */ (rows);
    ...
    const staticRows = /** @type {Types.MeasuredStaticRow[]} */ ([]);
    const addRow = (/** @type {number} */ snap, /** @type {Types.MeasuredStaticRow} */ row) => {
        staticRows.push(row);
        currentTop += row.height + borderWidth;
        globalRowIndex += snap;
    }
    const addStaticRow = (/** @type {Types.MeasuredStaticRow} */ row) => {
        log('static row');
        addRow(1, row);
    }
    const addDataRows = (/** @type {Types.MeasuredDynamicRow} */ row, /** @type {number} */ start, /** @type {number} */ end) => {
    ...
    const addSeparator = (/** @type {Types.MeasuredDynamicRow} */ row, /** @type {number} */ span) => {
