...
export function getVisibleRows(filters, filteringRules, formattingRules, data, rows, columns, edition) {
    if (filters.length === 0)
        return rows;

    const filterLookup = getFilterLookup(filters, 'columnId', 'rowId');
    const filteredColumns = columns.filter(column => filterLookup.has(column.key));
...
export function getVisibleColumns(filters, filteringRules, formattingRules, data, rows, columns, edition) {
    if (filters.length === 0)
        return columns;

    const filterLookup = getFilterLookup(filters, 'rowId', 'columnId');
    const filteredRows = rows.filter(row => filterLookup.has(row.key));
