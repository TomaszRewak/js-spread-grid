 ...
 * @param {Types.SortDirection[]} [sortOrder]
 * @returns {Types.SortBy[]}
 */
export default function getNewSortBy(sortBy, column, row, ctrlKey, sortOrder) {
    // TODO: better support for multi-row sorting

    /** @param {Types.SortBy} rule */
    ...
    const directionLoop = sortOrder
        ? sortOrder.map(direction => direction === null ? undefined : direction)
        : ['ASC', 'DESC', undefined];
