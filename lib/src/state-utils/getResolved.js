...
/**
 * @template {StaticDimension} T
 * @param {T} element
 * @param {number} index
 * @returns {T & ResolvedStaticDimension}
 */
function resolveStaticRow(element, index) {
...
/**
 * @template {DynamicDimension} T
 * @param {T} element
 * @param {number} index
 * @returns {T & ResolvedDynamicDimension}
 */
function getResolved(element, index) {
    const id = 'id' in element ? element.id : () => index;
...
export function getResolvedColumns(columns, columnWidths, borderWidth, devicePixelRatio) {
    ...
    return columns.map((column, index) => {
        if (column.type === 'DYNAMIC-BLOCK') {
            ...
            const resolved = resolveStaticRow(column, index);
...
export function getResolvedRows(rows, rowHeights, borderWidth, devicePixelRatio) {
    ...
    return rows.map((row, index) => {
        if (row.type === 'DYNAMIC-BLOCK') {
            ...
            const resolved = resolveStaticRow(row, index);
