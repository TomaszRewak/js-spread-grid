...
function resolveStaticDimension(element, index) {
    ...
    return {
        ...
        resizable: 'resizable' in element ? element.resizable : true,
 ...
 * @param {number} devicePixelRatio
 * @param {number} dynamicColumnWidth
 * @returns {Types.ResolvedColumn[]}
 */
export function getResolvedColumns(columns, columnWidths, borderWidth, devicePixelRatio, dynamicColumnWidth) {
 ...
 * @param {number} borderWidth
 * @param {number} devicePixelRatio
 * @param {number} dynamicRowHeight
 * @returns {Types.ResolvedRow[]}
 */
export function getResolvedRows(rows, rowHeights, borderWidth, devicePixelRatio, dynamicRowHeight) {
    ...
    return rows.map((row, index) => {
        if (row.type === 'DYNAMIC-BLOCK') {
            ...
            const rowHeight = 'height' in row ? row.height : dynamicRowHeight;
            const height = rowHeight * row.count + borderWidth * (row.count - 1);
            return {
                ...resolved,
                count: row.count,
                height: roundToPixels(height, devicePixelRatio),
                rowHeight: roundToPixels(rowHeight, devicePixelRatio)
            };
        } else {
