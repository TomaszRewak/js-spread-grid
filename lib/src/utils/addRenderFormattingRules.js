...
export default function addRenderFormattingRules(formatting, hoveredCell, focusedCell, selection, highlight) {
    ...
    const isHighlighted = (rows, columns, rowIndex, columnIndex) => {
        if (rowIndex < 0 || rowIndex >= rows.length)
            return false;
        if (columnIndex < 0 || columnIndex >= columns.length)
            return false;
        ...
        return highlight.isKeySelected(rowKey, columnKey);
    }
