export default function getSections(columns, rows) {
    const topLength = rows.findIndex(row => row.pinned !== 'BEGIN');
    const bottomLength = rows.length - rows.findLastIndex(row => row.pinned !== 'END') - 1;
    const middleLength = rows.length - topLength - bottomLength;
    const leftLength = columns.findIndex(column => column.pinned !== 'BEGIN');
    const rightLength = columns.length - columns.findLastIndex(column => column.pinned !== 'END') - 1;
