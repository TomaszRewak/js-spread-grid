...
function initialize(element) {
    ...
    function buildClickEventProperties(event)
    {
        return {
            ctrlKey: event.ctrlKey,
            shiftKey: event.shiftKey,
    ...
    context.addEventListener(element, 'contextmenu', (event) => {
        // Do not forward context menu events whilst editing cells
        ...
        if (!verifyClickCell(cell))
            return;

        const column = context.state.columnLookup.get(stringifyId(cell.columnId));
        const row = context.state.rowLookup.get(stringifyId(cell.rowId));
        const eventProperties = buildClickEventProperties(event);

        if (column.type === 'DATA' && row.type === 'DATA') {
            context.state.options.onCellContextMenu({ ...context.state.hoveredCell, ...eventProperties });
        } else if (column.type === 'CUSTOM' || row.type === 'CUSTOM') {
            context.state.options.onCustomCellContextMenu({ ...context.state.hoveredCell, ...eventProperties });
        }
    })
