...
import showError from "./core/error.js";
import showInput from "./core/input.js";
import updateCursor from "./core/cursor.js";
...
function updateGrid(context) {
    ...
    try {
        ...
        if (context.state.styleState !== context.lastStyleState) {
            context.lastStyleState = context.state.styleState;
            style(context);
        }

        if (context.state.renderState !== context.lastRenderState) {
            context.lastRenderState = context.state.renderState;
            render(context);
        }

        if (context.state.inputState !== context.lastInputState) {
            context.lastInputState = context.state.inputState;
            showInput(context);
        }

        if (context.state.cursorState !== context.lastCursorState) {
            context.lastCursorState = context.state.cursorState;
            updateCursor(context);
        }

        if (context.state.tooltipState !== context.lastTooltipState) {
            context.lastTooltipState = context.state.tooltipState;
            showTooltip(context);
        }

        if (context.state.scrollState !== context.lastScrollState) {
            context.lastScrollState = context.state.scrollState;
            scroll(context);
        }
    } catch (error) {
...
function initialize(element) {
    ...
    context.requestNewRender = () => {
        ...
        if (context.errorRendered) return;
    ...
    const accept = (autoCommit) => {
        ...
        const isTextValid = context.state.inputState.isTextValid;
    ...
    addEventListener(element, 'pointerdown', (event) => {
        ...
        if (context.state.cursorState.resizableColumn) {
            context.resizingColumn = context.state.cursorState.resizableColumn;
        }
        if (context.state.cursorState.resizableRow) {
            context.resizingRow = context.state.cursorState.resizableRow;
        }
    ...
    addEventListener(element, 'click', (event) => {
        ...
        if (context.state.cursorState.resizableColumn || context.state.cursorState.resizableRow)
            return;
        ...
        if (cellData.edit?.toggle) {
            ...
            const sortOrderCell = context.state.sortOrderFormatResolver.resolve(row, column);
            const newSortBy = getNewSortBy(sortBy, column, row, event.ctrlKey, sortOrderCell.sortOrder);
    ...
    addEventListener(element, 'dblclick', (event) => {
        ...
        if (context.state.cursorState.resizableColumn) {
            const resizableColumnKey = stringifyId(context.state.cursorState.resizableColumn);
        ...
        if (context.state.cursorState.resizableRow) {
            const resizableRowKey = stringifyId(context.state.cursorState.resizableRow);
    ...
    addEventListener(element, 'wheel', (event) => {
        if (context.state.scrollState.verticalTarget !== null || context.state.scrollState.horizontalTarget !== null) {
            element.scrollBy({
                top: event.deltaY,
                left: event.deltaX,
                behavior: 'auto'
