...
export class Context {
     ...
     * @param {Object.<SectionName, HTMLCanvasElement>} canvases
     * @param {HTMLInputElement} input
     * @param {HTMLElement} tooltip
     */
    constructor(element, canvases, input, tooltip) {
        ...
        this.localOptions = {
            ...
            onFocusedCellChange: (focusedCell) => {
                this.localOptions.focusedCell = focusedCell;
                this.requestNewRender();
            },
            selectedCells: [],
            onSelectedCellsChange: (selectedCells) => {
                this.localOptions.selectedCells = selectedCells;
                this.requestNewRender();
            },
            highlightedCells: [],
            editedCells: [],
            onEditedCellsChange: (editedCells) => {
                ...
                this.localOptions.editedCells = editedCells;
                this.requestNewRender();
            },
            filters: [],
            onFiltersChange: (filters) => {
                this.localOptions.filters = filters;
                this.requestNewRender();
            },
            sortBy: [],
            onSortByChange: (sortBy) => {
                this.localOptions.sortBy = sortBy;
                this.requestNewRender();
            },
            onCellClick: () => { },
            onCustomCellClick: () => { },
            ...
            onColumnWidthsChange: (columnWidths) => {
                this.localOptions.columnWidths = columnWidths;
                this.requestNewRender();
            },
            rowHeights: [],
            onRowHeightsChange: (rowHeights) => {
                this.localOptions.rowHeights = rowHeights;
                this.requestNewRender();
            },
            columnsOrder: [],
            onColumnsOrderChange: (columnsOrder) => {
                this.localOptions.columnsOrder = columnsOrder;
                this.requestNewRender();
            },
            rowsOrder: [],
            onRowsOrderChange: (rowsOrder) => {
                this.localOptions.rowsOrder = rowsOrder;
                this.requestNewRender();
            },
...
function initialize(element) {
    ...
    tooltip.setAttribute('style', 'pointer-events: none; background-color: white; color: black; border: 1px solid black; padding: 5px; inset: unset; position: absolute;');
    tooltip.setAttribute('popover', '');

    const context = new Context(element, canvases, input, tooltip);
    ...
    addEventListener(element, 'mouseenter', (event) => {
        context.mousePosition = getMousePosition(event);
        context.requestNewRender();
    });

    addEventListener(element, 'mousemove', (event) => {
        context.mousePosition = getMousePosition(event);
    ...
    addEventListener(element, 'mouseleave', () => {
    ...
    addEventListener(element, 'mouseup', (event) => {
        updateState(context);
    ...
    addEventListener(element, 'keydown', (event) => {
        ...
        /** @param {CellId} cell, @param {KeyboardEvent} event */
    ...
    addEventListener(input, 'mousedown', (event) => {
        event.stopPropagation();
    });
