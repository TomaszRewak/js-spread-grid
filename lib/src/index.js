...
function initialize(element) {
    ...
    context.localOptions = {
        data: [],
        columns: data => data.length > 0 ? Object.keys(data[0]).map(key => ({ id: key, header: key })) : [],
        rows: data => [{ type: 'HEADER' }, ...data.map((_, index) => ({ id: index }))],
        formatting: [],
        filtering: [],
        sorting: [],
    ...
    context.addEventListener(element, 'click', (event) => {
        ...
        if (column.type === 'DATA' && row.type === 'DATA') {
            ...
            const currentDirection = context.state.options.sortBy
                .find(sort => column.key === stringifyId(sort.columnId) && row.key === stringifyId(sort.rowId))
                ?.direction;

            if (currentDirection === 'ASC')
                context.state.options.onSortByChange([{ columnId: column.id, rowId: row.id, direction: 'DESC' }]);
            else if (currentDirection === 'DESC')
                context.state.options.onSortByChange([]);
            else
                context.state.options.onSortByChange([{ columnId: column.id, rowId: row.id, direction: 'ASC' }]);
        }
    });
